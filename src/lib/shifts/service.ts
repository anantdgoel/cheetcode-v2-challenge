...
import type { LandingView, ShiftView } from '@/lib/domain/views'
...
export async function getLandingView (
  githubValue: string | null | Promise<string | null>
): Promise<LandingView> {
  const leaderboardPromise: Promise<Awaited<ReturnType<typeof getPublicLeaderboard>>> =
    getPublicLeaderboard().catch(() => [])
  const github = await githubValue
  const currentShiftPromise = (async (): Promise<ShiftView | null> => {
    if (!github) return null
    try {
      return await getCurrentShiftForGithub(github)
    } catch {
      return null
    }
  })()
  const [leaderboard, currentShift] = await Promise.all([leaderboardPromise, currentShiftPromise])

  return {
    leaderboard,
    activeShiftId: currentShift?.id ?? null,
    github
  }
}
